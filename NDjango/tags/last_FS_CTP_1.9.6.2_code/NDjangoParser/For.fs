...
///    </ul>
    ...
    type Node(
        ...
        override this.walk walker = 
            ...
            |Some o ->
                match o with
                | :? IEnumerable as loop when not (loop |> Seq.cast |> Seq.is_empty) -> 
//                    let size =
//                        match loop with
...
//                    let size = Some ((loop.OfType<Object>()).Count<Object>())
                    ...
                    let createContext (first:bool) (walker:Walker) (item:obj) =
                        ...
                        let c = 
                            if List.length variables = 1 then
                                ...
                                match item with
                                | :? IEnumerable as list ->
                                    Seq.combine (Seq.cast list) variables |> Seq.fold 
    ...
    and Repeater(token:BlockToken, enumerator:obj seq, createWalker) =
        ...
        override this.walk(walker) =
            if Seq.is_empty enumerator then
                walker
            else 
    ...
    type Tag() =
        ...
        interface NDjango.Interfaces.ITag with 
            member this.Perform token parser tokens =
                ...
                let variables = variables |> List.rev |>  List.fold_left (fun l item -> (List.append l (Array.to_list( item.Split([|','|], StringSplitOptions.RemoveEmptyEntries))))) []  
