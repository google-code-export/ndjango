...
module internal Cycle =
    ...
    /// Cycles among the given strings each time this tag is encountered.
    type Node(token:BlockToken, name: string, value_list: string list) =
        inherit NDjango.Interfaces.Node(Block token)
        
        let values = List.map (fun v -> new Variable(Block token, v)) value_list
    ...
    type Tag() = 
        ...
        let checkForOldSyntax value = 
            if (String.IsNullOrEmpty value) then false
                    ...
                    | '"' -> false
                    | '\'' -> false
                    | _ when String.contains value ',' -> true
                    | _ -> false
                            
        let normalize values =
            if List.exists checkForOldSyntax values then
                let compacted = List.fold_left (fun status value -> status + value) "" values
