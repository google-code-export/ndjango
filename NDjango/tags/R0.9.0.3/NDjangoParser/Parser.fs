...
module internal Parser =
    ...
    type public DefaultParser(tags: Map<string, ITag>, filters: Map<string, ISimpleFilter>) =
        ...
        let rec parse_internal parser (nodes:Node list) (tokens : LazyList<Lexer.Token>) parse_until =
           match tokens with
           | LazyList.Nil | LazyList.Cons(Lexer.Nothing, _) -> 
                if not <| List.is_empty parse_until then
                    fail_unclosed_tags parse_until
                (nodes, LazyList.of_list [Lexer.Nothing])
           | LazyList.Cons(token, tokens) -> 
                if is_term_token token parse_until then
                    (nodes @ [new Node(token)], tokens)
                else
                    if (not <| List.is_empty parse_until) && LazyList.hd tokens = Lexer.Nothing && not <| is_term_token token parse_until then 
                        fail_unclosed_tags parse_until
                    else
        ...
        let rec seek_internal parse_until tokens = 
            match tokens with 
            | LazyList.Nil | LazyList.Cons(Lexer.Nothing, _) -> fail_unclosed_tags parse_until
            | LazyList.Cons(token, tokens) -> 
