            ...
            member this.Perform token parser tokens = 
                ...
                | parent::[] -> 
                    ...
                    let parent_name_expr = 
                        new FilterExpression(Block token, parent, (parser :?> IFilterManager))
                    
                    [(new NDjango.ASTLoader.ExtendsNode(token, node_list, parent_name_expr) :> Node)], seq [Nothing]
    ...
    type IncludeTag() =
        ...
        interface ITag with
            member this.Perform token parser tokens = 
                ...
                | name::[] -> 
                    let template_name = 
                        new FilterExpression(Block token, name, (parser :?> IFilterManager))
                    [{
...
/// See also: {% include %}.
    ...
    type SsiNode(token:BlockToken, reader: Reader) = 
        ...
        override this.walk walker =
            let templateReader =  
                match reader with 
                | Path path -> walker.context.Manager.GetTemplateReader(path)
            ...
            let nodes = 
                ...
                else Seq.cons (new SsiNode(token, TextReader templateReader) :> Node) walker.nodes
            {walker with buffer = Some buffer; nodes=nodes}
    ...
    type SsiTag() =
        ...
        interface ITag with
            member this.Perform token parser tokens = 
                ...
                | path::"parsed"::[] ->
                    let templateRef = FilterExpression (Block token, ("\"" + path + "\"") , (parser :?> IFilterManager))
                    [{
                        new Node(Block token) 
                        with
                            override this.walk walker = 
