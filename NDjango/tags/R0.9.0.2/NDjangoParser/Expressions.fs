...
module Expressions =
    ...
    type FilterExpression (token:Lexer.Token, expression: string, filter_accessor: IFilterManager) =
        ...
        let rec parse_var (filter_match: Match) upto (filter_accessor: IFilterManager) (var: Option<string>) =
            if not (filter_match.Success) then
                ...
                let fast_call = fun v -> parse_var (filter_match.NextMatch()) (filter_match.Index + filter_match.Length) filter_accessor v
                ...
                if not (upto = filter_match.Index) then
                    ...
                    | Some s ->
                        ...
                        let filter = filter_accessor.FindFilter filter_name.Value
        ...
        let upto, variable, filters = parse_var (Constants.filter_re.Match(expression)) 0 filter_accessor None
        ...
        member this.ResolveForOutput walker =
            ...
            | Some o -> 
                ...
                | _ as v ->
                    ...
                    | "" -> None
                    | _ as s -> Some {walker with buffer = Some s}
