...
module internal Parser =
    ...
    type public DefaultParser(tags: Map<string, ITag>, filters: Map<string, ISimpleFilter>) =
        ...
        let rec parse_internal parser (nodes:Node list) (tokens : Lexer.Token seq) parse_until =
            ...
            if token = Lexer.Nothing then  
                ...
                if is_term_token token parse_until then
                    ...
                    if (not <| List.isEmpty parse_until) && Seq.hd tokens = Lexer.Nothing && not <| is_term_token token parse_until then 
                        fail_unclosed_tags parse_until
                    else
        ...
        let rec seek_internal parser parse_until tokens = 
            if Seq.isEmpty tokens then None
            else
                let h = Seq.hd tokens
