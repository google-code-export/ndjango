...
module internal ASTWalker =
    type Reader(walker: Walker) =
        ...
        let rec getChar() = 
            ...
            | None ->
                match walker.nodes |> Seq.tryPick (fun node -> Some node)  with
                | Some node ->
                    walker <- {walker with nodes = Seq.skip 1 walker.nodes}  // advance node list
        ...
        override this.Read(buffer: char[], index: int, count: int) = 
        ...
        override this.ReadToEnd() =
            ...
            match this.Read(buffer, 0, buffer |> Array.length) with
            | 0 -> ""
            | _ as l -> (new System.String(buffer.[..l-1])) + this.ReadToEnd()
