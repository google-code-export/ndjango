    ...
    type Tag() =
        interface ITag with
            member this.Perform token parser tokens =
                ...
                let createWalker =
                    match token.Args |> List.map (fun var -> new Variable(Block token, var)) with
                    | [] ->
                        fun walker ->
                            let reader = 
                            ...
                            | Some o when o = newValue -> {walker with nodes = Seq.append nodes_ifsame walker.nodes}
                            | _ -> {walker with buffer=Some (string newValue); context=walker.context.add("$oldValue", newValue)}
                    | _ as vars ->
                        fun walker ->
                            ...
                            match walker.context.tryFind("$oldValue") with
                            | Some o when not <| matchValues o newValues -> {walker with nodes = Seq.append nodes_ifsame walker.nodes}
                            | _ -> {walker with nodes = Seq.append nodes_ifchanged walker.nodes; context=walker.context.add("$oldValue", (newValues :> obj))}
                ([{
