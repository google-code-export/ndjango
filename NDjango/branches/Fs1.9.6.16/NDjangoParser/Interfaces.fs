...
module Interfaces = 
    ...
    /// Template imeplementation. This interface effetively represents the root-level node
    /// in the Django AST.
    and ITemplate =
    ...
    and IParser =
        ...
        abstract member Parse: Lexer.Token seq -> string list -> (Node list * Lexer.Token seq)
       
        /// Produces an uncommited token list as a result of parsing until
        /// a block from the string list is encotuntered
        abstract member Seek: Lexer.Token seq -> string list -> Lexer.Token seq

    /// A single tag implementation
    and ITag = 
        /// Transforms a {% %} tag into a list of nodes and uncommited token list
        abstract member Perform: Lexer.BlockToken -> IParser -> Lexer.Token seq -> (Node list * Lexer.Token seq)
