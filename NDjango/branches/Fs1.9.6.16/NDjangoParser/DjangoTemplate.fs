...
module Template =
    ...
    type Manager 
        ...
        /// the instance of the template manager that contains the template
            ...
            member this.RenderTemplate (full_name, context) = 
                let manager, template = (this :> ITemplateManager).GetTemplate full_name
    ...
    and private Impl(template, manager: Manager) =
        ...
        let node_list =
            ...
            let parser = new Parser.DefaultParser(manager.Tags, manager.Filters) :> IParser
            // this will cause the TextReader to be closed when the template goes out of scope
            use template = template
            // It is ncessary to cache the sequence produced by the tokenize function
            // otherwise the Seq.isEmpty will mess it up - it creates its own IEnumerator over the same sequence
