...
module Template =
    ...
    type Manager 
        ...
        /// 
        member private this.GetAndReloadIfNeeded full_name template ts managerSetter = 
            if loader.IsUpdated (full_name, ts) then
                lock lockMgr 
                    (fun () -> (!active).RegisterTemplate full_name (loader.GetTemplate full_name) managerSetter) 
            else
                template
        ...
        member this.GetTemplate full_name managerSetter =
            match Map.tryFind full_name templates with
            | Some (template, ts) -> 
                this.GetAndReloadIfNeeded full_name template ts managerSetter
            | None -> 
                lock lockMgr 
                    (fun () -> 
                        let mgr = !active
                        match Map.tryFind full_name mgr.Templates with
                        | Some (template, ts) -> this.GetAndReloadIfNeeded full_name template ts managerSetter
                        | None -> mgr.RegisterTemplate full_name (loader.GetTemplate full_name) managerSetter
                    )

        interface ITemplateContainer with
            member this.RenderTemplate (full_name, context, managerSetter) = 
                let setterInvoker = managerSetter.Invoke
                let template = this.GetTemplate full_name setterInvoker
                template.Walk context setterInvoker
        ...
        /// Creates a new filter manager with the tempalte registered 
        member private this.RegisterTemplate name template managerSetter =
    ...
    and private Impl(template, manager: Manager) =
        ...
        interface ITemplate with
            member this.Walk context managerSetter =
                new ASTWalker.Reader (
                    {parent=None; 
                     nodes=node_list; 
                     buffer="";
                     bufferIndex = 0; 
                     context=new Context(manager, context, (new Map<string,obj>(context |> Seq.map (fun item-> (item.Key, item.Value)))), managerSetter)
                    }) :> System.IO.TextReader
                
            member this.Nodes = node_list
            
    and
        private Context (manager: ITemplateManager, externalContext: obj, variables: Map<string, obj>, managerSetter: ITemplateManager->unit, ?autoescape: bool) =
        ...
        interface IContext with
            member this.add pair =
                new Context(manager, externalContext, Map.add (fst pair) (snd pair) variables, managerSetter, autoescape) :> IContext
            ...
            member this.GetTemplate template = 
                ...
                let ctx = ref (this)
                let setterWrapper manager =
                    ctx := new Context(manager, externalContext, variables, managerSetter)
                    managerSetter(manager)
                let a = setterWrapper
                !ctx :> IContext, (manager :?> Manager).GetTemplate template setterWrapper
            ...
            member this.WithAutoescape(value) =
                new Context(manager, externalContext, variables, managerSetter, value) :> IContext
